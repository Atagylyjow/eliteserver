...
document.querySelectorAll('.unlock-btn').forEach(btn => {
    btn.addEventListener('click', function() {
        ...
        if (scriptId === 'shadowsocks') {
            // Shadowsocks için fetch ile içerik göster
            fetch('https://raw.githubusercontent.com/Atagylyjow/vpnserver/blob/main/shadowsocks.txt')
                .then(response => response.text())
                .then(text => showTextModal(text))
                .catch(() => alert('Shadowsocks dosyası alınamadı!'));
        } else if (downloadUrl) {
...
// Kopyalanabilir modal fonksiyonu
function showTextModal(text) {
    // Eski modalı kaldır
    const oldModal = document.getElementById('ss-modal');
    if (oldModal) oldModal.remove();
    // Modal oluştur
    const modal = document.createElement('div');
    modal.id = 'ss-modal';
    modal.style.cssText = `
        position: fixed; left: 0; top: 0; width: 100vw; height: 100vh; background: rgba(0,0,0,0.5); z-index: 9999; display: flex; align-items: center; justify-content: center;`;
    modal.innerHTML = `
        <div style="background: #fff; padding: 2rem; border-radius: 12px; max-width: 90vw; max-height: 80vh; box-shadow: 0 8px 32px rgba(0,0,0,0.2); display: flex; flex-direction: column; align-items: stretch;">
            <h2 style="margin-bottom: 1rem; color: #333;">Shadowsocks Script</h2>
            <textarea readonly style="width: 400px; max-width: 80vw; height: 200px; max-height: 40vh; margin-bottom: 1rem; padding: 1rem; border-radius: 8px; border: 1px solid #ccc; font-size: 1rem;">${text}</textarea>
            <button id="copy-ss-btn" style="padding: 0.5rem 1.5rem; border-radius: 8px; background: #007bff; color: #fff; border: none; font-size: 1rem; margin-bottom: 0.5rem; cursor: pointer;">Kopyala</button>
            <button id="close-ss-btn" style="padding: 0.5rem 1.5rem; border-radius: 8px; background: #ccc; color: #333; border: none; font-size: 1rem; cursor: pointer;">Kapat</button>
        </div>
    `;
    document.body.appendChild(modal);
    document.getElementById('copy-ss-btn').onclick = function() {
    ...
    document.getElementById('close-ss-btn').onclick = function() {
        modal.remove();
    };
} 